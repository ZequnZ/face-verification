...
from evaluation import load_embeddings
import facenet

logging.basicConfig(level=logging.DEBUG)


def init_facenet(model_path="../model/keras/model/facenet_keras.h5"):
    """Initialize FaceNet model and return"""

    # print("Creating networks and loading parameters...")
    print("Initialize FaceNet........")
...
def img2emb_keras(img, model=None):
    """cropped face imgs -> embeddings"""

    # if the model is not passed
    if model is None:
        model_path = "../model/keras/model/facenet_keras.h5"
        model = load_model(model_path)
    emb = model.predict_on_batch(img)
...
while 1:
    ...
    if DRAWBOX and bounding_boxes is not None:
        for box, img in zip(bounding_boxes, images):
            ...
            emb = img2emb_keras(img, model)

            for key in save_emb:
                dist = np.sqrt(np.sum(np.square(np.subtract(emb, save_emb[key]))))
                print(f"dist: {round(dist,4)}")
                if dist < 1:
                    image = cv2.putText(
                        image, key, start_point, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2,
                    )

            image = cv2.putText(
                image, "test", start_point, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2
            )
